...
## Local TensAI bot notes

Quick notes to run locally for playground testing:

- Local environment files are under `env/`. Edit `env/.env.local` to set secrets or use the included values. Important keys: `PORT=8000`, `M365_AGENT_ENABLED=true`, `MICROSOFT_APP_ID`, `MICROSOFT_APP_PASSWORD`.
- Start the app with the VS Code task "Start application" or run `npm run dev:teamsfx` after `npm install`.
- The bot replies with a main menu when users send `hi`, `hello` or `menu`. Selecting a module (WebGPT, Media Studio, OmniQuest, Translator, Summarizer) shows feedback buttons (thumbs up / thumbs down) which are persisted to `src/feedback_store.json`.
